...
fn main() {
    let wire1: Wire = Wire {name: String::from("W1"), points: vec![Vector3::new(0.0,0.0,0.0), Vector3::new(1.0,2.0,3.6)]};
    let point: Point = Point { name: String::from("P1"), cords: Vector3::new(0.0, 23.3, 21.1) };

    println!("Wire {} points {:?}", wire1.name, wire1.points);
    println!("Test cross product: {:?}",  wire1.points[1].cross(&point.cords));
